// doubao_batch_v8.js - 剪贴板粘贴(一次到位) + 每次重选图像生成模式
...
async function clickImageGen(page) {
  return await page.evaluate(() => {
    for (const b of document.querySelectorAll('button'))
      if ((b.textContent.includes('图像生成') || b.textContent.trim() === '图像生成') && b.offsetParent !== null) { b.click(); return true; }
    return false;
  });
}

async function pasteText(page, text) {
  // 剪贴板写入 + Ctrl+V，真正的一次性粘贴
...
async function focusInput(page) {
  return await page.evaluate(() => {
    for (const el of document.querySelectorAll('[contenteditable], textarea')) {
      if (el.offsetParent !== null) { el.focus(); if (el.isContentEditable) el.innerHTML = ''; return true; }
    }
    return false;
  });
}
...
async function extractImages(page) {
  return await page.evaluate(() =>
    Array.from(document.querySelectorAll('img'))
      .map(i => i.src).filter(s => s.includes('byteimg'))
  );
}
...
async function main() {
  ...
  let page = (await browser.pages()).find(p => p.url() === 'https://www.doubao.com/chat/' || p.url().startsWith('https://www.doubao.com/chat/'));
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      // 1. 点击"图像生成"按钮确保模式正确
      const clicked = await clickImageGen(page);
      if (!clicked) console.log(`  ⚠️ 图像生成按钮未找到`);
      await sleep(1500);
      
      // 2. 聚焦输入框并清空
      const focused = await focusInput(page);
      await sleep(300);
      
      // 3. Ctrl+A + Delete 清空已有内容
      ...
      // 4. 剪贴板粘贴（一次性、不分段）
      await pasteText(page, e.fullPrompt);
      ...
      // 6. 提取图片
      let urls = await extractImages(page);
      
      if (!urls.length) {
        console.log(`  ⏳ 再等20s...`);
        await sleep(20000);
        urls = await extractImages(page);
