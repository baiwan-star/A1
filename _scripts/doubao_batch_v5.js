// doubao_batch_v5.js - 使用 CDP Input.insertText 直接粘贴
...
async function main() {
  ...
  const btnClicked = await page.evaluate(() => {
    ...
    for (const b of btns) {
      if (b.textContent.includes('图像生成') && b.offsetParent !== null) {
        b.click(); return true;
      }
    }
    return false;
  });
  console.log(`图像生成按钮: ${btnClicked ? '已点击' : '未找到'}`);
  await sleep(2000);
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      ...
      // 第三步：使用 CDP 的 Input.insertText 直接插入（最快方式）
      const cdp = await page.target.createCDPSession();
      await cdp.send('Input.insertText', { text: e.fullPrompt });
      ...
      if (imageUrls.length === 0) {
        ...
        if (retryUrls.length > 0) imageUrls.push(...retryUrls);
        else { console.log(`  ❌ 无图`); continue; }
      }
